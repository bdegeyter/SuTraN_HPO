    ...
    def forward(self, 
        ...
        if self.training: # Teacher forcing (for now)
            ...
            for dec_layer in self.decoder_layers:
                dec_output, _ = dec_layer(dec_output, x, padding_mask_input)
            ...
            # Initialize KV caches for each decoder layer
            layer_caches = [None] * self.num_decoder_layers

            for dec_step in range(0, window_size):
                # Embed only the new token at position dec_step
                cat_emb_suf = self.act_emb(act_inputs[:, dec_step:dec_step+1]) # (B, 1, activity_emb_size)
                new_time_feats = num_ftrs_suf[:, dec_step:dec_step+1, :] # (B, 1, 2)
                new_token = torch.cat((cat_emb_suf, new_time_feats), dim=-1) # (B, 1, dim_init_suffix)

                # Project to d_model, scale, and add positional encoding
                new_token = self.input_embeddings_decoder(new_token) * math.sqrt(self.d_model) # (B, 1, d_model)
                new_token = new_token + self.positional_encoding.pe[dec_step:dec_step+1, :] # (B, 1, d_model)
                if self.layernorm_embeds:
                    new_token = self.norm_dec_embeds(new_token) # (B, 1, d_model)

                # Pass through decoder layers with KV caching
                dec_output = new_token
                for i, dec_layer in enumerate(self.decoder_layers):
                    dec_output, layer_caches[i] = dec_layer(dec_output, x, padding_mask_input, cache=layer_caches[i])

                # Prediction heads on dec_output: (B, 1, d_model)
                ...
                if dec_step == 0:
                    if self.remaining_runtime_head:
                        rrt_pred = self.fc_out_rrt(dec_output) # (B, 1, 1)
                        rrt_pred = rrt_pred[:, 0, 0] # (B,)

                    if self.outcome_bool:
                        out_pred = self.fc_out_out(dec_output) # (B, 1, 1)
                        out_pred = self.sigmoid_out(out_pred) # (B, 1, 1)
                        out_pred = out_pred[:, 0, 0] # (B,)
