...
def prepare_bac_log(df):
    ...
    # Compute remaining time to scheduled departure (minutes) as a per-event
    # numeric feature before dropping the raw datetime column.
    dep_time = pd.to_datetime(df['dep_scheduled_deptime'], format='mixed', utc=True)
    df['remaining_deptime_min'] = (dep_time - df['time:timestamp']).dt.total_seconds() / 60.0
...
def construct_BAC_dep_datasets():
    ...
    numeric_eventfeatures = ['remaining_deptime_min']
